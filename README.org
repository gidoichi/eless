...
cat ./eless | awk '/^* Usage Examples$/{f=1} /"/{f=0} f' | tail +3 | sed 's/^ *//'
#+end_src

#+RESULTS:
#+begin_src shell
eless foo.txt                         # Open foo.txt in eless.
echo 'foo' | eless                    #
...
PAGER=eless man grep                  # Launches man pages in eless, if the env var PAGER is set to eless (does not work on macOS).
PAGER=less man -P eless grep          # Launches man pages in eless, if the env var PAGER is *not* set to eless (works on macOS).
#+end_src
